...
for sentences in data:
	text = ''.join([word for word in sentences.split() if word not in cachedStopWords])
	corpus.append(text)

bigram_vectorizer = CountVectorizer(ngram_range=(1, 2), token_pattern=r'\b\w+\b', min_df=1)
x_data = bigram_vectorizer.fit_transform(data).toarray()
...
with open('AllTopicstfidf.csv', 'rb') as csvfile:
